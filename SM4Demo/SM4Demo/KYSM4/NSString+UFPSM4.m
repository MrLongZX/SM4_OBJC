...
@implementation NSString (UFPSM4)
...
- (nullable NSString *)encryptionWithSM4Key:(nonnull NSString *)secretKey iv:(nonnull NSString *)iv padding:(UFPPaddingMode)padding {
    ...
    int paddingLength = padding == UFPPaddingMode_PKCS7 ? 16 - (int)msgLength % 16 + (int)msgLength : (int)msgLength;
    ...
    if (padding == UFPPaddingMode_PKCS7) {
    ...
    //使用0，表示不限制一行最大长度，不插入\r或\n
    NSString *base64string = [outdata base64EncodedStringWithOptions:0];
...
- (nullable NSString *)decryptionWithSM4Key:(nonnull NSString *)secretKey iv:(nonnull NSString *)iv padding:(UFPPaddingMode)padding {
    ...
    unsigned long stringLength = padding == UFPPaddingMode_PKCS7 ? 0 : msgLength;
    if (padding == UFPPaddingMode_PKCS7) {
